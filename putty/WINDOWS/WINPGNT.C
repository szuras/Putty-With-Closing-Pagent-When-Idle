...
DWORD request_ui_get_idle_time()
{
...
DWORD WINAPI threadProc()
{
	...
	if (user32 == NULL){
		...
		if (user32 == NULL)
		{
			MessageBox(NULL,"failed to load user32","problem",MB_OK);
			exit(0);
		}
	}

	if (!(getLastInputInfo = (BOOL (WINAPI *)(PLASTINPUTINFO))GetProcAddress(user32, "GetLastInputInfo"))){
			MessageBox(NULL,"failed to look up GetLastInputInfo","problem",MB_OK);
			exit(0);
	}
	...
	for (i = 0; ; i++)
    {
		...
		if ( (GetTickCount() - info.dwTime) / 1000 > max_idle_value * 60)
		{
			exit(0);
		}
        Sleep (1000);
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	if (!strcmp(argv[i], "-pgpfp")) {
		...
		if (i < argc-1){
			i++;
			/*
			 * If we use `-i' with eg. 30 value, then the agent will be killed 
			 * after the 30 minutes of the computer idle
			 */
			max_idle_value = atoi(argv[i]);
		}
	} else {
